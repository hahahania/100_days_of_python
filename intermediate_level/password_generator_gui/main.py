...
web = tk.Label(text='Website: ', fg=FONT_COLOR,
               bg=BG_COLOR, font=(FONT, 15)).grid(column=1, row=2, sticky='E', pady=5)

web_name = tk.Entry(window, bg=INPUT_COLOR,
                    borderwidth=0.5, highlightthickness=0, fg=FONT_COLOR, font=(FONT, 15)).grid(column=2, columnspan=2, row=2, sticky='WE')

name = tk.Label(text='Email / Nickname: ', fg=FONT_COLOR,
                bg=BG_COLOR, font=(FONT, 15)).grid(column=1, row=3, sticky='E', pady=5)

user_name = tk.Entry(window, bg=INPUT_COLOR,
                     borderwidth=0.5, highlightthickness=0, fg=FONT_COLOR, font=(FONT, 15)).grid(column=2, columnspan=2, row=3, sticky='WE')

password = tk.Label(text='Password: ', fg=FONT_COLOR,
                    bg=BG_COLOR, font=(FONT, 15)).grid(column=1, row=4, sticky='E', pady=5)

user_password = tk.Entry(window, bg=INPUT_COLOR, textvariable=v,
                         borderwidth=0.5, highlightthickness=0, fg=FONT_COLOR, font=(FONT, 15)).grid(column=2, row=4,)

generate_password = tk.Button(window, text='Generate Password', bg=INPUT_COLOR,
                              borderwidth=0.25, highlightthickness=0, fg=FONT_COLOR, font=(FONT, 15), command=new_password).grid(column=3, row=4)

submit = tk.Button(window, text='Submit', bg=INPUT_COLOR,
                   borderwidth=0.25, highlightthickness=0, fg=FONT_COLOR, font=(FONT, 15), command=collect_data).grid(row=5, column=2, pady=20)
